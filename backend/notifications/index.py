        ...
        if action == "subscribe_push" and method == "POST":
            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    f"VALUES (%s, %s, %s, %s) ON CONFLICT (user_id, endpoint) DO UPDATE SET p256dh = EXCLUDED.p256dh, auth = EXCLUDED.auth",
                    (user["id"], endpoint, p256dh, auth)
                )
            conn.commit()
            return {"statusCode": 200, "headers": CORS, "body": json.dumps({"ok": True})}

        # POST unsubscribe_push — удалить push-подписку
        if action == "unsubscribe_push" and method == "POST":
            endpoint = body.get("endpoint", "")
            with conn.cursor() as cur:
                cur.execute(
                    f"UPDATE {SCHEMA}.push_subscriptions SET endpoint = endpoint WHERE user_id = %s AND endpoint = %s",
                    (user["id"], endpoint)
                )
            conn.commit()
            return {"statusCode": 200, "headers": CORS, "body": json.dumps({"ok": True})}

        # POST send — создать уведомление (только для админа или системы)
            ...
            with conn.cursor() as cur:
                if broadcast:
                    cur.execute(
                        ...
                        f"SELECT id, %s, %s, %s, %s FROM {SCHEMA}.users",
                        (notif_type, title, notif_body, json.dumps(data) if data else None)
                    )
                    affected = cur.rowcount
                elif target_user_id:
                    cur.execute(
                        f"INSERT INTO {SCHEMA}.notifications (user_id, type, title, body, data) VALUES (%s, %s, %s, %s, %s)",
                        (target_user_id, notif_type, title, notif_body, json.dumps(data) if data else None)
                    )
                    affected = 1
                else:
